...
serve(async (req) => {
  ...
  try {
    ...
    if (!videoId) return new Response(JSON.stringify({ error: "videoId required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    // Security: Check if user is enrolled in the course that owns this video, OR if they are admin
    const { data: part } = await admin.from('parts').select('id, is_preview, chapters!inner(subjects!inner(course_id))').eq('video_id', videoId).maybeSingle();
    
    if (!part) return new Response(JSON.stringify({ error: "Part not found" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    return new Response(JSON.stringify({ otp: vdoData.otp, playbackInfo: vdoData.playbackInfo }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err: any) {
