...
-- 3.1 profiles -------------------------------------------------------
-- UPDATED: Removed bio, added address & personal details
...
-- ADDED: Idempotency index for Complete Button random rewards
CREATE UNIQUE INDEX IF NOT EXISTS uniq_ledger_complete_part
  ON public.coin_ledger(user_id, source, ref_id)
  WHERE source = 'complete';
...
-- ---------------------------------------------------------------------
-- 6.2 ADDED: PRODUCTION-READY ATOMIC GAMIFICATION FUNCTION
-- Handles: XP, Coins, Level, Streak Decay, Concurrency
-- Decay: 1 day missed = 25% reduce, 2 days = 50% reduce, 7+ days = 0
-- ---------------------------------------------------------------------
...
RETURNS TABLE (xp integer, coins integer, level integer, current_streak integer, longest_streak integer)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ DECLARE
  ...
  v_gap integer;
BEGIN
  -- 1. Lock the row to prevent concurrent race conditions (100% safe for multiple tabs/users)
  SELECT p.xp, p.coins, p.last_activity_date, p.current_streak, p.longest_streak
  INTO v_xp, v_coins, v_last_date, v_streak, v_longest
  FROM public.profiles p WHERE p.user_id = _user_id
  FOR UPDATE;

  -- 2. Calculate the gap since last activity
  IF v_last_date IS NULL THEN
    v_gap := 999; -- First time ever
  ELSE
    v_gap := (v_today - v_last_date)::integer;
  END IF;

  -- 3. Apply Streak Decay Logic (Based on days missed)
  IF v_gap >= 7 THEN
    -- 7+ days missed: Streak goes to 0
    v_streak := 0;
  ELSIF v_gap = 3 THEN
    -- 2 days missed: 50% reduce
    v_streak := FLOOR(COALESCE(v_streak, 0) * 0.5)::integer;
  ELSIF v_gap = 2 THEN
    -- 1 day missed: 25% reduce
    v_streak := FLOOR(COALESCE(v_streak, 0) * 0.75)::integer;
  END IF;
  -- v_gap = 0 (already active today) or 1 (active yesterday): No penalty!

  -- 4. Increment streak if this is the first action of a new day
  IF v_gap >= 1 THEN
    v_streak := COALESCE(v_streak, 0) + 1;
  END IF;

  -- 5. Add rewards
  v_xp     := COALESCE(v_xp, 0) + _xp;
  v_coins  := COALESCE(v_coins, 0) + _coins;
  v_level  := GREATEST(1, FLOOR(SQRT(v_xp / 100.0))::integer + 1);
  ...
  RETURN QUERY SELECT v_xp, v_coins, v_level, v_streak, v_longest;
END;
 $$;
...
-- =====================================================================
-- 14. PROFILE FEATURE MIGRATION (Update Existing Table)
-- =====================================================================
-- This section ensures existing databases get the new columns without data loss.

-- 1. Remove the old 'bio' column if it exists
ALTER TABLE public.profiles DROP COLUMN IF EXISTS bio;

-- 2. Add new personal details columns
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS gender text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS date_of_birth date;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS language text;

-- 3. Add new address columns
...
-- Add the new video_provider column to your parts table
ALTER TABLE public.parts 
ADD COLUMN IF NOT EXISTS video_provider text NOT NULL DEFAULT 'bunny';

-- (Optional but recommended) Ensure live_chat_enabled exists too
ALTER TABLE public.parts 
ADD COLUMN IF NOT EXISTS live_chat_enabled boolean NOT NULL DEFAULT false;
